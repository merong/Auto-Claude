    ...
    <ScrollArea className="h-full">
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-foreground">{t('index.title')}</h2>
          ...
          <Tooltip>
            ...
            <TooltipContent>{t('index.refreshTooltip')}</TooltipContent>
          </Tooltip>
        </div>
        ...
        {indexError && (
          <div className="flex items-center gap-3 p-4 rounded-lg bg-destructive/10 text-destructive">
            ...
            <div>
              <p className="font-medium">{t('index.loadFailed')}</p>
        ...
        {!indexLoading && !projectIndex && !indexError && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground">{t('index.noIndexTitle')}</h3>
            <p className="text-sm text-muted-foreground mt-2 max-w-sm">
              {t('index.noIndexDescription')}
            </p>
            <Button onClick={onRefresh} className="mt-4">
              <RefreshCw className="h-4 w-4 mr-2" />
              {t('index.analyzeProject')}
            </Button>
          </div>
        )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {Object.keys(projectIndex.services).length > 0 && (
                    <Badge variant="secondary">
                      {t('index.serviceCount', { count: Object.keys(projectIndex.services).length })}
                    </Badge>
                  )}
                </div>
                  ...
                  <CardContent className="pt-6">
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      ...
                      {projectIndex.conventions.typescript && (
                        <InfoItem label={t('index.typescript')} value={t('index.enabled')} />
                      )}
                    </div>
                  </CardContent>
                </Card>
