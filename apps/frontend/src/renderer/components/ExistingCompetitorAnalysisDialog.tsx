...
export function ExistingCompetitorAnalysisDialog({
  ...
  const { t, i18n } = useTranslation(['ideation', 'common']);
  ...
  const formatDate = (date?: Date) => {
    if (!date) return t('competitorDialog.recently');
      ...
      <AlertDialogContent className="sm:max-w-[500px]">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-muted-foreground">
            {t('competitorDialog.existingAnalysis', { date: formatDate(analysisDate) })}
          </AlertDialogDescription>
        </AlertDialogHeader>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-medium text-foreground flex items-center gap-2">
                  {t('competitorDialog.useExisting')}
                  <span className="text-xs text-primary font-normal">({t('competitorDialog.recommended')})</span>
                </h4>
                <p className="text-xs text-muted-foreground mt-1">
                  {t('competitorDialog.useExistingDesc')}
                </p>
              </div>
            </div>
          </button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-medium text-foreground">
                  {t('competitorDialog.runNew')}
                </h4>
                <p className="text-xs text-muted-foreground mt-1">
                  {t('competitorDialog.runNewDesc')}
                </p>
              </div>
            </div>
          </button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-sm font-medium text-muted-foreground">
                  {t('competitorDialog.skip')}
                </h4>
                <p className="text-xs text-muted-foreground/80 mt-1">
                  {t('competitorDialog.skipDesc')}
                </p>
              </div>
            </div>
          </button>
